...
def process_image(img: Image.Image, format_key: str, hashtag: str) -> Image.Image:
    ...
    if hashtag and hashtag != "— Без хештега —":
        sample_x = max(0, canvas_w - HASHTAG_RIGHT - 200)
        ...
        # Масштабируем размер шрифта пропорционально ширине канваса (базовый размер для 1920px)
        scaled_size = int(HASHTAG_SIZE * canvas_w / 1920)
        ...
        # Letter spacing -0.7% от размера шрифта
        spacing = int(scaled_size * (-0.007))

        # Рисуем посимвольно для letter spacing
        # Сначала считаем общую ширину
        total_w = 0
        char_widths = []
        for ch in hashtag:
            ...
            total_w += w + spacing
        total_w -= spacing  # убираем лишний spacing после последнего символа

        tx = canvas_w - HASHTAG_RIGHT - total_w
        ty = canvas_h - HASHTAG_BOTTOM - scaled_size
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Отправь мне фото (можно сразу несколько), затем /done"
    )
